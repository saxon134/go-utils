...
func RoundYuan(yuan interface{}, roundType ...RoundType) float64 {
	...
	var fenDecimal = yuanDecimal.Mul(decimal.NewFromInt(100))
...
func Qian2Yuan(qian interface{}, roundType ...RoundType) (yuan float64) {
	...
	if round == RoundTypeDefault {
		if f-float64(i) >= 0.5 {
			f, _ = decimal.NewFromInt(i + 1).Div(decimal.NewFromInt(10000)).Float64()
			return f
		} else {
			f, _ = decimal.NewFromInt(i).Div(decimal.NewFromInt(10000)).Float64()
			return f
		}
	} else
	//向上取整
...
func Yuan2Qian(yuan interface{}, roundType ...RoundType) (li int64) {
	...
	switch v := yuan.(type) {
	case int:
		liDecimal = decimal.NewFromFloat(float64(v)).Mul(decimal.NewFromInt(10000))
	case int64:
		liDecimal = decimal.NewFromFloat(float64(v)).Mul(decimal.NewFromInt(10000))
	case string:
		d, _ := decimal.NewFromString(v)
		f, _ := d.Float64()
		liDecimal = decimal.NewFromFloat(f).Mul(decimal.NewFromInt(10000))
	case float32:
		liDecimal = decimal.NewFromFloat(float64(v)).Mul(decimal.NewFromInt(10000))
	case float64:
		liDecimal = decimal.NewFromFloat(v).Mul(decimal.NewFromInt(10000))
	case int32:
		liDecimal = decimal.NewFromFloat(float64(v)).Mul(decimal.NewFromInt(10000))
	default:
		s := fmt.Sprint(yuan)
